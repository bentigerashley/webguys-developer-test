...
export function ServicesSection({ block }: { block: ServicesBlock }) {
  ...
  return <section id="services" className="section services expertise" aria-labelledby="services-heading"><div className="page-grid">
    <header className="component-header services-header"><p className="section-index">Expertise</p><div className="services-title-group"><h2 id="services-heading">{block.heading}</h2><span className="services-count" aria-label={`${block.items.length} services`}>[{block.items.length}]</span></div></header>
    <div className="services-support"><p className="services-intro">{block.intro}</p>{block.detail && <p className="services-detail">{block.detail}</p>}<a className="button figma-cta" href="#contact">Learn More<span aria-hidden="true">→</span></a></div>
    <p className="services-list-label">Services:</p>
    ...
    <Reveal className="service-panel"><div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${active}`} tabIndex={0}><SafeImage image={item.image} /><div className="service-detail-copy"><div><h3>{item.title}</h3><p className="eyebrow">Service</p></div><p>{item.body}</p></div></div></Reveal>
  </div></section>;
}

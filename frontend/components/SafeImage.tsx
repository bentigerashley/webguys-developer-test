...
export function SafeImage({ image, className = "", eager = false }: { image: ImageValue; className?: string; eager?: boolean }) {
  ...
  return <div className={`safe-image ${failed ? "is-fallback" : ""} ${className}`}>
    {!failed && <img src={image.url} alt={image.alt} loading={eager ? "eager" : "lazy"} fetchPriority={eager ? "high" : "auto"} onError={() => setFailedUrl(image.url)} style={{ objectPosition: image.position ?? "center" }} />}
